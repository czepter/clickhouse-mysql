...
fork of official [yandex/clickhouse-server](https://hub.docker.com/r/yandex/clickhouse-server/) with mysql port enabled and exposed.
...
```bash
$ docker run -d --name some-clickhouse-server --ulimit nofile=262144:262144 czepter/clickhouse-mysql
```
...
```bash
$ docker run -d -p 18123:8123 -p19000:9000 --name some-clickhouse-server --ulimit nofile=262144:262144 czepter/clickhouse-mysql
...
```bash
$ docker run -d --network=host --name some-clickhouse-server --ulimit nofile=262144:262144 czepter/clickhouse-mysql
...
$ docker run -d \
	...
	--name some-clickhouse-server --ulimit nofile=262144:262144 czepter/clickhouse-mysql
```
...
Container exposes 8123 port for [HTTP interface](https://clickhouse.yandex/docs/en/interfaces/http_interface/) and 9000 port for [native client](https://clickhouse.yandex/docs/en/interfaces/tcp/) and 9004 port for [mysql client](https://clickhouse.tech/docs/en/interfaces/mysql/).
...
```bash
$ docker run -d --name some-clickhouse-server --ulimit nofile=262144:262144 -v /path/to/your/config.xml:/etc/clickhouse-server/config.xml czepter/clickhouse-mysql
```

### Start server as custom user
```
# $(pwd)/data/clickhouse should exist and be owned by current user
$ docker run --rm --user ${UID}:${GID} --name some-clickhouse-server --ulimit nofile=262144:262144 -v "$(pwd)/logs/clickhouse:/var/log/clickhouse-server" -v "$(pwd)/data/clickhouse:/var/lib/clickhouse" czepter/clickhouse-mysql
```
...
```
$ docker run --rm -e CLICKHOUSE_UID=0 -e CLICKHOUSE_GID=0 --name clickhouse-server-userns -v "$(pwd)/logs/clickhouse:/var/log/clickhouse-server" -v "$(pwd)/data/clickhouse:/var/lib/clickhouse" czepter/clickhouse-mysql
```
...
```
$ docker run --rm -e CLICKHOUSE_DB=my_database -e CLICKHOUSE_USER=username -e CLICKHOUSE_PASSWORD=password -p 9000:9000/tcp czepter/clickhouse-mysql
```
